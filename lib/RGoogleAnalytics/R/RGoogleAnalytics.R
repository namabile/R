...
RGoogleAnalytics <- function() {
  ...
  CheckAuthTokenRenew <- function() {
    ## private helper function that checks to see if we have a valid Google auth token on the network
    ## test for age of auth token, if older than 2 weeks get new
    if (file.exists(token.path)){

      file.token <- file.info(token.path)
      age <- difftime(Sys.time(), file.token[["mtime"]], unit = "days")
      if(age > 14 ){
          auth.token <<- NULL
      }
      else {
          x <- scan(token.path, what = character(), quiet=TRUE)
          if(!is.null(x[2])) {
            auth.token <<- x[2]
          }
          else {
  ...
  SetCredentials <- function(username, pass) {
   ...
   auth.resp <- postForm("https://www.google.com/accounts/ClientLogin",
    ...
    sink(file = token.path)
    print(as.character(auth.token))
    sink()
